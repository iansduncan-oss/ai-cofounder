...
describe("approval timeout sweep", () => {
  it("resolves expired pending approvals", async () => {
    ...
    const expired = await listExpiredPendingApprovals({} as import("@ai-cofounder/db").Db);
    for (const approval of expired) {
      await resolveApproval({} as import("@ai-cofounder/db").Db, approval.id, "rejected", "Auto-denied: approval timeout exceeded");
    }
