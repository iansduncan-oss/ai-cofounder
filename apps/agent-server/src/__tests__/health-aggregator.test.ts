...
beforeEach(() => {
  vi.clearAllMocks();
  // Default: REDIS_URL is set, other external env vars not set
...
describe("GET /health/full", () => {
  it("returns 200 with status ok when all core services healthy", async () => {
    ...
    expect(body.core.database).toBe("ok");
    expect(body.core.redis).toBe("ok");
  ...
  it("returns 503 with status degraded when DB is down", async () => {
    ...
    expect(body.core.database).toBe("unreachable");
  });
  ...
  it("returns 503 with status degraded when Redis is unreachable", async () => {
    ...
    const body = res.json();
    expect(body.status).toBe("degraded");
    expect(body.core.redis).toBe("unreachable");
  });

  it("reports LLM provider status from registry", async () => {
    const { app } = buildServer();
    // Default mock returns [] so llm.status should be "none"
    const res = await app.inject({ method: "GET", url: "/health/full" });
    await app.close();

    const body = res.json();
    expect(body.llm.status).toBe("none");
    expect(body.llm.available).toBe(0);
    expect(body.llm.total).toBe(0);
  });

  it("detects external service config from env vars", async () => {
    // Set GitHub and VPS env vars
    mockOptionalEnv.mockImplementation((name: string, defaultValue: string) => {
      const envMap: Record<string, string> = {
        REDIS_URL: "redis://localhost:6379",
        GITHUB_TOKEN: "ghp_test123",
        GITHUB_MONITORED_REPOS: "owner/repo",
        VPS_HOST: "168.119.162.59",
        VPS_USER: "ian",
      };
      return envMap[name] ?? defaultValue;
    });
