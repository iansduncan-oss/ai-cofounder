...
function makeService(): BudgetAlertService {
  return new BudgetAlertService(mockDb, mockNotificationService as any);
}
...
describe("BudgetAlertService", () => {
  ...
  it("returns suggestion when claude-opus used for >10 requests", async () => {
    mockGetUsageSummary.mockResolvedValue({
      ...
      byModel: {
        ...
        "gpt-4o-mini": { inputTokens: 10000, outputTokens: 5000, costUsd: 1.0, requests: 35 },
      },
      byAgent: {},
    });
    const svc = makeService();
    ...
    expect(
      suggestions.some(
        (s) => s.toLowerCase().includes("opus") || s.toLowerCase().includes("expensive"),
      ),
    ).toBe(true);
  });
  ...
  it("returns 'no opportunities' when usage is modest", async () => {
    mockGetUsageSummary.mockResolvedValue({
      ...
      byProvider: {},
      byModel: {
        "claude-haiku-3": { inputTokens: 1000, outputTokens: 500, costUsd: 0.1, requests: 5 },
      },
      byAgent: {},
    });
    const svc = makeService();
  ...
  it("returns suggestion when orchestrator agent has very high cost", async () => {
    mockGetUsageSummary.mockResolvedValue({
      ...
      byModel: {},
      byAgent: {
        orchestrator: { inputTokens: 40000, outputTokens: 15000, costUsd: 15.0, requests: 80 },
        coder: { inputTokens: 10000, outputTokens: 5000, costUsd: 5.0, requests: 20 },
      },
    });
    const svc = makeService();
    const suggestions = await svc.generateOptimizationSuggestions();
    ...
    expect(
      suggestions.some(
        (s) => s.toLowerCase().includes("orchestrator") || s.toLowerCase().includes("agent"),
      ),
    ).toBe(true);
  });
});
